...
import { LANGUAGES, DEFAULT_MESSAGES, Language } from '../constants/chatbotMessage';
import { FaPaperPlane, FaRobot, FaUser, FaMicrophone, FaStop, FaTimes, FaQuestionCircle } from 'react-icons/fa';
...
const Chatbot = ({ from, chatId, selectedBank } : { from: string, chatId: string, selectedBank: BankProps }) => {

  const [currentLanguage, setCurrentLanguage] = useState<Language>("TRADITIONAL_CHINESE");
  const suggestions = DEFAULT_MESSAGES(selectedBank)[currentLanguage].suggestions;
  ...
  useEffect(() => {
    setMessages([{
      ...
      text: DEFAULT_MESSAGES(selectedBank)[currentLanguage].text,
      lang: currentLanguage,
      isUser: false,
      followUpQuestion: DEFAULT_MESSAGES(selectedBank)[currentLanguage].suggestions.map(
        (suggestion) => suggestion.replace("?", `${Math.floor(Math.random() * 5) + 1}.HK`)
      ),
      showFollowUp: true
    }]);
  }, []);
  ...
  const toggleLanguage = () => {
    const languages: Language[] = ['TRADITIONAL_CHINESE', 'SIMPLIFIED_CHINESE', 'ENGLISH'];
    ...
    setMessages(prevMessages => {
      if (prevMessages.length > 0) {
        ...
        newMessages[0] = {
          ...
          text: DEFAULT_MESSAGES(selectedBank)[nextLanguage].text,
          lang: nextLanguage,
          isUser: false,
          followUpQuestion: DEFAULT_MESSAGES(selectedBank)[nextLanguage].suggestions.map(
            (suggestion) => suggestion.replace("?", `${Math.floor(Math.random() * 5) + 1}.HK`)
          ),
          showFollowUp: true
        };
  ...
  const sendMessage = async (text?: string) => {
    ...
    if ((!messageContent && !audioBlob && !audioUrl) ||
      (!messageContent && audioBlob && !audioUrl) ||
      (!messageContent && !audioBlob && audioUrl)) return;
    ...
    try {
      ...
      const botResponse = response.output || "Sorry, I could not process that.";
      const followUp = response.question ? response.question.split("*") : [];

      if (response.isPassToWealth && response.query) {
    ...
    return (
      <div className="bg-gray-100 border-l-4 border-blue-500 pl-4 py-3 my-4 text-gray-600 text-sm md:text-md truncate">
        {parentMessage.audioUrl
          ? <audio controls src={parentMessage.audioUrl} className="h-10 w-48 sm:w-64" />
    ...
    if (message.isLoading) {
      return (
        <div className="flex gap-2 pt-3">
          ...
          <div className="w-2 h-2 bg-teal-400 rounded-full opacity-75 animate-bounce" style={{ animationDelay: '0.2s' }}></div>
          <div className="w-2 h-2 bg-teal-400 rounded-full opacity-75 animate-bounce" style={{ animationDelay: '0.4s' }}></div>
        </div>
      );
    }
    return (
  ...
  useEffect(() => {
    messageQueue.subscribeWealth((newMessage: RetailMessage) => {
  ...
  return (
    <div className="flex flex-col overflow-y-auto flex-1">
      <div
        ...
        {messages.filter((message) => message.chatbotId === chatId).map((message, idx) => (
          <div
            key={idx}
            className={`max-w-[95%] sm:max-w-[85%] p-3 sm:p-4 mb-3 sm:mb-4 relative animate-fadeIn ${message.isUser
          ...
          >
            ...
            <div className={`flex items-center gap-2 mb-2 text-sm sm:text-base font-bold ${message.isUser ? 'text-blue-100' : 'text-blue-600'
              }`}>
              {message.isUser ? <FaUser /> : <FaRobot className='pb-1' />}
              <span>{message.isUser ? 'You' : 'Finbot AI'}</span>
            </div>

            {displayMessage(message)}

            {!message.isUser && !message.isLoading && message.showFollowUp && message.followUpQuestion && (
        ...
        {isRecording ? (
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && !isMessageSending && sendMessage()}
              placeholder="Ask about markets news, stocks, currencies..."
              className={`w-full ${LANGUAGES[currentLanguage] === LANGUAGES.ENGLISH ? "px-16 py-2 sm:px-18 sm:py-3" : "px-12 py-2 sm:px-14 sm:py-3"} border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent text-sm sm:text-base`}
            />
